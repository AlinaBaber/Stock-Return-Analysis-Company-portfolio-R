...
#net sales or revenues (WC01001) minus cost ofgoods sold (WC01501)


test <- panel_c %>% select(Id ,Date, YEAR, month, ym, WC02999, WC01250, WC03501, UP, RET.USD, MV.USD_lag, WC01551, WC03263, book_market, pf.size, pf.value, WC01501, WC01001) %>% na.omit()
...
RM <- aggregate(test.new[,20:21], list(test.new$Date), sum) #Taking yearly average for market return


library(readxl)
Model <- read_excel("Data Returns and KF Returns.xlsx")
...
ggplot(Model, aes(Model$Date , Model$HML )) +
  geom_line()
ggplot(Model, aes(Model$Date , Model$HML )) +
  geom_col()
ggplot(Model, aes(Model$Date , Model$`HML (KF)` )) +
  geom_line()
ggplot(Model, aes(Model$Date , Model$`HML (KF)` )) +
  geom_col()
ggplot(Model, aes(Model$Date , Model$SMB )) +
  geom_line()
ggplot(Model, aes(Model$Date , Model$`SMB (KF)` )) +
  geom_line()
ggplot(Model, aes(Model$Date , Model$RMRF )) +
  geom_line()
ggplot(Model, aes(Model$Date , Model$`Mkt-RF (KF)` )) +
  geom_line()

summary(Model$HML)

Model <- Model %>% select(Date, RF ,`Mkt-RF (KF)`, `HML (KF)`, `SMB (KF)`, RMRF, SMB, HML)
...
test$grossprofits <- (test$WC01001) - (test$WC01501)
summary(test$grossprofits)

test$bookequity <- (test$WC03501) + (test$WC03263)
summary(test$bookequity)

test$returnonequity <- (test$WC01551)/(test$bookequity)
summary(test$returnonequity)

Calculation <- test %>% select(Id, Date, YEAR, pf.value, returnonequity, bookequity, grossprofits, OP_BE, WC02999) %>% na.omit()
summary(Calculation)

is.na(Calculation) <- sapply(Calculation, is.infinite)

summary(Calculation)

ProfitabilityFactorCalculations <- na.omit(Calculation)
summary(ProfitabilityFactorCalculations)
...
library(magrittr)
 ...
 str(ProfitabilityFactorCalculations)
